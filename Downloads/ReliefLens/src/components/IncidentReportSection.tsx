...
export const IncidentReportSection: React.FC<IncidentReportSectionProps> = ({
  ...
  const inputKey = `${photoBase64?.slice(0, 40) || ''}|${voiceTranscript}|${textInput}`;
  ...
  const runAnalysis = useCallback(
    async (openChat = false, force = false) => {
      ...
      const draft: DraftIncident = { photoBase64, voiceTranscript, textInput };
    ...
    [isInputEmpty, inputKey, photoBase64, voiceTranscript, textInput, userLocation, onIncidentExtracted]
  );
  ...
  useEffect(() => {
    if (isInputEmpty || photoBase64) return;
    const hasEnoughText = (voiceTranscript + textInput).trim().length >= 15;
    if (!hasEnoughText) return;

    const timer = setTimeout(() => runAnalysis(true), 2500);
    return () => clearTimeout(timer);
  }, [voiceTranscript, textInput, isInputEmpty, runAnalysis, photoBase64]);
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
            ...
            <div className="flex flex-col gap-6">
              <VoiceNoteCapture onTranscriptUpdate={setVoiceTranscript} transcript={voiceTranscript} />
